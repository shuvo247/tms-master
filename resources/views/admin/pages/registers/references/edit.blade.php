                ...
                <div class="repeater-default m-t-30">
                    <div data-repeater-list="">
                        <div data-repeater-item="">
                            <form action="{{route('register.reference.update',['reference_id' => $reference->id])}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-row">
                                    <div class="form-group col-md-3">
                                        ...
                                        <select class="form-control select-2" id="organization" name="organization_id">
                                            @foreach (App\Models\Organization::all() as $organization)
                                                <option value="{{$organization->id}}" @if($organization->id == $reference->organization_id) selected @endif>{{$organization->organization_name}}</option>
                                            @endforeach
                                          </select>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="referrerName">Referrer Name <span class="text-danger">*</span></label>
                                        <input name="referrer_name" type="text" class="form-control" id="referrerName" value="{{$reference->referrer_name ?? ''}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="address">Address <span class="text-danger">*</span></label>
                                        <input name="address" type="text" class="form-control" id="address" value="{{$reference->address ?? ''}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="mobileNumber">Mobile Number <span class="text-danger">*</span></label>
                                        <input name="mobile_number" type="text" class="form-control" id="mobileNumber" value="{{$reference->mobile_number ?? ''}}">
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="form-group col-md-3">
                                        <label for="alternativeMobile">Alternative Mobile <span class="text-danger">*</span></label>
                                        <input name="alternative_mobile_number" type="text" class="form-control" id="alternativeMobile" value="{{$reference->alternative_mobile_number ?? ''}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <!-- <img src="{{asset('uploads/'.$reference->image)}}" width="80px" alt=""> -->
