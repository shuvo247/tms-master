                ...
                <div class="repeater-default m-t-30">
                    <div data-repeater-list="">
                        <div data-repeater-item="">
                            <form action="{{route('register.organization.update',['organization_id' => $organization->id])}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="form-row">
                                    <div class="form-group col-md-3">
                                        <label for="supplierName">Select Type <span class="text-danger">*</span></label>
                                        <select class="form-control select-2" id="supplierName" name="organization_id">
                                            @foreach (App\Models\SupplierType::all() as $type)
                                                <option value="{{$type->id}}" @if($type->id == $organization->organization_type) selected @endif>{{$type->supplier_type}}</option>
                                            @endforeach
                                          </select>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="supplierName">Organization Name <span class="text-danger">*</span></label>
                                        <input name="organization_name" type="supplierName" class="form-control" id="text" value="{{$organization->organization_name ?? ''}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="owner_name">Owner Name <span class="text-danger">*</span></label>
                                        <input name="owner_name" type="text" class="form-control" id="owner_name" value="{{ $organization->owner_name ?? ''}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label for="address">Address <span class="text-danger">*</span></label>
                                        <textarea name="address" class="form-control" id="address" rows="1">{{ $organization->address ?? ''}}</textarea>
                                    </div>
                                </div>
                                <div class="form-row">
                                    <div class="form-group col-md-3">
                                        <label for="mobileNumber">Mobile Number <span class="text-danger">*</span></label>
                                        <input name="mobile_number" type="text" class="form-control" id="mobileNumber" value="{{$organization->phone ?? ''}}">
                                    </div>
                                    <div class="form-group col-md-3">
                                        <img src="{{asset('uploads/'.$organization->trade_licence)}}" alt="">
